...
/**
 * CLASE PRINCIPAL: SistemaPOS
 * 
 * Esta es la clase principal que orquesta todo el sistema POS (Point of Sale).
 * Maneja la interacción con el usuario a través de un menú de consola y coordina
 * todas las operaciones del sistema.
 * 
 * RELACIONES:
 * - ASOCIACIÓN/DEPENDENCIA con todas las clases: SistemaPOS crea y usa todas las demás
 *   clases del sistema, pero no las almacena como atributos de clase. Solo las usa
 *   en métodos locales (relación de dependencia temporal).
 * 
 * RESPONSABILIDADES:
 * - Inicializar todos los componentes del sistema (Inventario, Caja, Reportes, Mesas, Empleado)
 * - Cargar productos iniciales en el inventario
 * - Mostrar menú principal y gestionar opciones del usuario
 * - Coordinar el flujo completo de toma de pedidos
 * - Gestionar el estado de las mesas
 * - Coordinar la generación de facturas y registro de pagos
 * 
 * FLUJO PRINCIPAL:
 * 1. Inicializa componentes (Inventario, Caja, Reportes, Mesas, Empleado)
 * 2. Carga productos iniciales
 * 3. Muestra menú principal con opciones:
 *    - Gestión de mesas
 *    - Tomar pedido
 *    - Ver inventario
 *    - Ver total en caja
 *    - Reportes diario/mensual
 * 4. Maneja la creación de pedidos y el flujo completo de venta
 * 
 * PATRÓN: Clase principal (Main class) - Punto de entrada del sistema
 */
public class SistemaPOS {
    ...
    public static void main(String[] args) {
        ...
        do {
            ...
            try {
                ...
                Logger.error(e, "Error en el menú principal");
     ...
     * Permite ver estado, reservar, ocupar y liberar mesas.
     * 
     * @param mesas Array de mesas del restaurante
     */
     ...
     * 
     * @param n Número de mesa a validar
     * @param mesas Array de mesas
    ...
    /**
     * Método principal para tomar un pedido completo.
     * 
     * FLUJO COMPLETO:
     * 1. Selecciona mesa y valida que esté disponible
     * 2. Crea cliente con nombre ingresado
     * 3. Crea pedido asociando Cliente, Empleado y Mesa (agregación)
     * 4. Permite agregar productos del inventario al pedido
     * 5. Genera factura usando Factura.imprimir() (asociación)
     * 6. Registra pago en Caja (asociación)
     * 7. Registra venta en Reportes diario y mensual (asociación)
     * 8. Cierra el pedido liberando la mesa
     * 
     * RELACIONES INVOLUCRADAS:
     * - Crea Cliente (heredado de Persona)
     * - Crea Pedido con Cliente, Empleado y Mesa (agregación)
     * - Usa Inventario para obtener productos (composición Inventario-Producto)
     * - Pedido agrega Productos (agregación)
     * - Usa Factura para imprimir (asociación)
     * - Usa Caja para registrar pago (asociación)
     * - Usa Reportes para acumular ventas (asociación)
     * 
     * @param mesas Array de mesas disponibles
     * @param inventario Inventario con productos disponibles
     * @param caja Caja registradora para registrar pagos
     * @param reporteDiario Reporte diario para acumular ventas
     * @param reporteMensual Reporte mensual para acumular ventas
    ...
    private static void tomarPedido(Mesa[] mesas,
        ...
        // NOTA: El constructor de Pedido automáticamente ocupa la mesa
        ...
        while (seguir) {
            ...
            // NOTA: Este método descontará stock automáticamente si hay disponible
            try {
        ...
        if (pagar.equalsIgnoreCase("s")) {
            try {
                ...
                try {
                    ...
                    Logger.error(e, "Error al guardar inventario después de venta");
                }
